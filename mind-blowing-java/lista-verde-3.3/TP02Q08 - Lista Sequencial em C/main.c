...
void print_serie(Serie *serie) {
  printf("%s %s %s %s %s %s %s %d %d\n", serie->nome, serie->formato,
         serie->duracao, serie->pais, serie->idioma, serie->emissora,
         serie->transmissao, serie->num_temporadas, serie->num_episodios);
}
...
void II(String nomeArquivoSerie){
...
void IF(String nomeArquivoSerie){
        Serie novaSerie = new Serie();
        novaSerie.buscarDados(nomeArquivoSerie);
...
void I(String nomeArquivoSerie, int posicao){
        Serie novaSerie = new Serie();
        novaSerie.buscarDados(nomeArquivoSerie);
...
int main() {
  ...
  for (int i = 0; i < quantidadeDeInstrucoes; i++) {
    char instrucaoLinha[MAX_LINE_SIZE];
    readline(instrucaoLinha, MAX_LINE_SIZE);

    if( instrucaoLinha[0] == 'I' ){
        if( instrucaoLinha[1] == 'I' ){
            //insere inicio
        }else if( instrucaoLinha[1] == 'F' ){
            // insere posicao especifica
        }else if( instrucaoLinha[1] == '*' ){
            // insere posicao especifica
        }
    }else if( instrucaoLinha[0] == 'R' ){
        if( instrucaoLinha[1] == 'I' ){
            // Remove inicio
        }else if( instrucaoLinha[1] == 'F' ){
