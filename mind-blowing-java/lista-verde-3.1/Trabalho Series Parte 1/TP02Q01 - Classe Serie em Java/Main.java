...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        String[] listaDeSeries = new String[1000];
        int index = 0;
        do {
            listaDeSeries[index] = stdin.readLine();
            index++;
        } while (isFim(listaDeSeries[index - 1]) == false);
        index--;

        for( int i = 0 ; i < index ; i++ ){
            Serie linhaSerie = new Serie();
            linhaSerie.buscarDados(listaDeSeries[i]);
        }
