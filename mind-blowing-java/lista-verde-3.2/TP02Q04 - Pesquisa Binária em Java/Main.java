...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < secondIndex; i++) {
            comparacoes++;
            int resultado = pesquisaBinaria(listaDadosSeries, nomeSerie[i], firstIndex);
            if( resultado != -1 ){
                stdout.println("SIM");
            }else{
                stdout.println("NÃO");
    ...
    public static int pesquisaBinaria(Serie[] listaDadosSerie, String nomeDaSerie, int lenDados){
        int primeiro = 0, ultimo = lenDados - 1;
        while (primeiro <= ultimo) {
            int meio = primeiro + (ultimo - primeiro) / 2;
            int res = nomeDaSerie.compareTo(listaDadosSerie[meio].getNome());
            if (res == 0){
                return meio;
            }
            if (res > 0){
